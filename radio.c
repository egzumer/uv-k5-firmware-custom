...
void RADIO_InitInfo(VFO_Info_t *pInfo, const channel_t ChannelSave, const uint32_t Frequency)
{
	...
	if (ChannelSave == (VFO_CHANNEL_FIRST + BAND2_108MHz))
...
void RADIO_ConfigureChannel(const unsigned int VFO, const unsigned int configure)
{
	...
	if (!gSetting_350EN) {
		if (gEeprom.FreqChannel[VFO] == VFO_CHANNEL_FIRST + BAND5_350MHz)
			gEeprom.FreqChannel[VFO] = VFO_CHANNEL_FIRST + BAND6_400MHz;

		if (gEeprom.ScreenChannel[VFO] == VFO_CHANNEL_FIRST + BAND5_350MHz)
			gEeprom.ScreenChannel[VFO] = VFO_CHANNEL_FIRST + BAND6_400MHz;
	}
	...
	if (att.__val == 0xFF) { // invalid/unused channel
		...
		uint8_t bandIdx = channel - VFO_CHANNEL_FIRST;
	...
	if (IS_MR_CHANNEL(channel)) {
		...
		band = channel - VFO_CHANNEL_FIRST;
	...
	uint16_t base;
	if (IS_MR_CHANNEL(channel))
		base = EEPROM_MR_FREQ_OFF + channel * 16;
	else
		base = EEPROM_VFO_FREQ_OFF + ((channel - VFO_CHANNEL_FIRST) * 32) + (VFO * 16);

	if (configure == VFO_CONFIGURE_RELOAD || IS_VFO_CHANNEL(channel))
	{
		uint8_t tmp;
		uint8_t data[8];
	...
	else if (channel >= VFO_CHANNEL_FIRST)
